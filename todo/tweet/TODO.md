...
- [x] (P1) [2025-01-27] #12: JavaDoc для эндпоинта — Добавить JavaDoc для всех новых методов
  acceptance: "JavaDoc добавлен для всех методов эндпоинта с @author geron, @version 1.0, @param, @return, @throws"
  note: "Проверена JavaDoc документация для всех методов эндпоинта getTimeline. Все методы имеют полную JavaDoc документацию: TweetApi#getTimeline (добавлены @throws теги), TweetService#getTimeline, TweetValidator#validateForTimeline, TweetServiceImpl#getTimeline (@see), TweetController#getTimeline (@see), TweetValidatorImpl#validateForTimeline (@see), FollowerGateway#getFollowingUserIds, FollowerApiClient#getFollowing. Все методы соответствуют стандартам STANDART_JAVADOC.md. Файлы: services/tweet-api/src/main/java/com/twitter/controller/TweetApi.java"
- [x] (P1) [2025-01-27] #13: Unit тесты для эндпоинта — Написать unit тесты для Service и Validator
  ...
  note: "Добавлены unit тесты для метода getTimeline в TweetServiceImplTest (6 тестов) и для метода validateForTimeline в TweetValidatorImplTest (3 теста). Все тесты следуют стандартам STANDART_TEST.md, используют паттерн именования methodName_WhenCondition_ShouldExpectedResult, используют @Nested для группировки, проверяют успешные и ошибочные сценарии. Тесты проверяют взаимодействие с зависимостями через verify. Файлы: services/tweet-api/src/test/java/com/twitter/service/TweetServiceImplTest.java, services/tweet-api/src/test/java/com/twitter/validation/TweetValidatorImplTest.java"
- [x] (P2) [2025-01-27] #14: Integration тесты для эндпоинта — Написать integration тесты для контроллера
  acceptance: "Integration тесты для эндпоинта с MockMvc, WireMock для follower-api, все статус-коды проверены (200, 400)"
  note: "Добавлены integration тесты для эндпоинта getTimeline в TweetControllerTest (9 тестов). Тесты используют MockMvc и WireMock для мокирования follower-api и users-api. Добавлены helper методы в BaseIntegrationTest для настройки follower-api stubs (setupFollowingStub, setupFollowingStubEmpty, setupFollowingStubWithError). Тесты проверяют: успешный сценарий с твитами, пустая лента (нет подписок), пустая лента (нет твитов), невалидный UUID, пользователь не существует, недоступность follower-api (graceful degradation), исключение удаленных твитов, сортировка по createdAt DESC, пагинация. Файлы: services/tweet-api/src/test/java/com/twitter/controller/TweetControllerTest.java, services/tweet-api/src/test/java/com/twitter/testconfig/BaseIntegrationTest.java"
- [x] (P1) [2025-01-27] #15: Swagger документация для эндпоинта — Добавить полную OpenAPI документацию
  acceptance: "OpenAPI документация для эндпоинта полная с @ExampleObject для всех сценариев (успех, пустая лента, ошибки)"
  note: "Проверена OpenAPI документация для эндпоинта getTimeline. Документация полная и соответствует стандартам STANDART_SWAGGER.md. Включены @ExampleObject для всех сценариев: успешный ответ с твитами (200), пустая лента без подписок (200), пустая лента без твитов (200), невалидный UUID (400), пользователь не существует (400), невалидные параметры пагинации (400), недоступность follower-api (503). Все примеры используют реалистичные данные и соответствуют формату RFC 7807 Problem Details для ошибок. Файл: services/tweet-api/src/main/java/com/twitter/controller/TweetApi.java"

### Финальная инфраструктура
- [x] (P2) [2025-01-27] #16: Обновление README.md — Добавить описание эндпоинта и интеграции с follower-api
  ...
  note: "Обновлен README.md для tweet-api. Добавлено описание эндпоинта getTimeline в раздел 'Основные возможности', таблицу эндпоинтов, детальное описание эндпоинта с примерами запросов и ответов. Добавлен раздел 'Интеграция с follower-api' с описанием архитектуры интеграции, компонентов (FollowerApiClient, FollowerGateway), процесса получения ленты новостей, обработки ошибок. Обновлена диаграмма компонентов и структура пакетов. Добавлен метод getTimeline в раздел бизнес-логики. Добавлены примеры использования getTimeline с curl командами. Обновлены требования к окружению. Файл: services/tweet-api/README.md"
- [x] (P2) [2025-01-27] #17: Обновление Postman коллекции — Добавить запрос get timeline
  ...
  note: "Добавлен запрос 'get timeline' в Postman коллекцию tweet-api. Запрос использует GET метод, путь /api/v1/tweets/timeline/{{userId}} с query параметрами page, size, sort. Добавлены примеры ответов для всех сценариев: успешный ответ с твитами (200 OK), пустая лента без подписок (200 OK), пустая лента без твитов (200 OK), невалидный UUID (400 Bad Request), пользователь не существует (400 Bad Request), невалидные параметры пагинации (400 Bad Request). Обновлено описание коллекции с упоминанием timeline эндпоинта и интеграции с follower-api. Используются переменные {{baseUrl}} и {{userId}}. Файл: postman/tweet-api/twitter-tweet-api.postman_collection.json"
- [x] (P1) [2025-01-27] #18: Проверка соответствия стандартам — Проверить соответствие всем стандартам проекта
  ...
  note: "Проверено соответствие всех стандартов проекта для эндпоинта getTimeline. STANDART_CODE.md: ✅ Используется @LoggableRequest, @RequiredArgsConstructor, @Slf4j, @Transactional, Gateway pattern, Feign Client, Records, MapStruct, валидация, Pageable, PagedModel. STANDART_PROJECT.md: ✅ Используется @LoggableRequest, GlobalExceptionHandler, ValidationException hierarchy, shared DTOs из common-lib. STANDART_JAVADOC.md: ✅ Все классы и методы имеют JavaDoc с @author, @version, @param, @return, @throws, используется @see для реализаций. STANDART_TEST.md: ✅ Тесты используют правильное именование, @Nested, @ExtendWith, AssertJ, WireMock, покрывают все сценарии, используют паттерн AAA. STANDART_SWAGGER.md: ✅ Проверено на шаге #15. STANDART_README.md: ✅ Проверено на шаге #16. STANDART_POSTMAN.md: ✅ Проверено на шаге #17. STANDART_DOCKER.md: ⏳ Будет проверено на шаге #19."
- [x] (P1) [2025-01-27] #19: Проверка Docker конфигурации — Проверить соответствие docker-compose.yml стандартам
  ...
  note: "Проверено соответствие Docker конфигурации стандартам STANDART_DOCKER.md. docker-compose.yml: ✅ версия 3.8, ✅ правильные имена сервисов и контейнеров, ✅ build конфигурация, ✅ порты (8082 для tweet-api), ✅ environment переменные (включая FOLLOWER_API_URL), ✅ depends_on с health conditions (добавлена зависимость от follower-api), ✅ networks (twitter-network), ✅ volumes (logs), ✅ restart (unless-stopped), ✅ healthcheck. Dockerfile для tweet-api: ✅ multi-stage build, ✅ build stage (gradle:jdk24), ✅ runtime stage (eclipse-temurin:24-jre), ✅ non-root user (appuser), ✅ healthcheck, ✅ expose 8082, ✅ JVM options, ✅ entrypoint. .dockerignore: ✅ все необходимые исключения. Добавлена зависимость tweet-api от follower-api в depends_on для правильного порядка запуска. Файлы: docker-compose.yml, services/tweet-api/Dockerfile, services/tweet-api/.dockerignore"
