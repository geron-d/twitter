...
**Follower API** — это микросервис для управления отношениями подписки (follow relationships) между пользователями в
системе Twitter, построенный на Java 24 и Spring Boot 3. Сервис предоставляет REST API для создания и удаления подписок,
получения списков подписчиков и подписок с пагинацией и фильтрацией, проверки статуса подписки и получения статистики
подписок.
...
3. **Существование пользователей:**
    ...
    - При нарушении выбрасывается `BusinessRuleValidationException` с правилом "FOLLOWER_NOT_EXISTS" или "
...
3. **`validateUsersExist(UUID followerId, UUID followingId)`** (private)
    ...
    - Выбрасывает `BusinessRuleValidationException` с правилом "FOLLOWER_ID_NULL", "FOLLOWING_ID_NULL", "
      FOLLOWER_NOT_EXISTS" или "FOLLOWING_NOT_EXISTS"
...
Follower API интегрируется с Users API через Feign Client для проверки существования пользователей и получения их
логинов. Интеграция необходима, так как информация о пользователях хранится в отдельном сервисе.
